...
function SignInPage() {
  ...
  const createUserWithEmailAndPassword = (e) => {
    ...
    auth.createUserWithEmailAndPassword(email, password).then((result) => {
      dispath(setActiveUser({ userEmail: result.user.email, userPassword: result.user.password }))
      toast.success('You are sign up succesfull.')
    }).catch((error) => {
      const errorCode = error.code;
      const errorMessage = error.message;
      if (errorCode === 'auth/weak-password') { setPasswordError(true); setPasswordErrorMessage('The password is too weak') }
      if (errorCode === 'auth/invalid-email') { setEmailError(true); setEmailErrorMessage('Email is invalid') }
      if (errorCode === 'auth/email-already-in-use') { setEmailError(true); setEmailErrorMessage('The email already in use , please login') }
  ...
  const SignInUserWithEmailAndPassword = (e) => {
    ...
    auth.signInWithEmailAndPassword(email, password).then((userCredential) => {
      ...
      const errorCode = error.code;
      const errorMessage = error.message;
      if (errorCode === "auth/user-not-found") { setEmailError(true); setEmailErrorMessage('User not found'); }
      if (errorCode === "auth/wrong-password") { setPasswordError(true); setPasswordErrorMessage('Password is wrong'); }
      else { toast.error(errorMessage) }
    })
  }
                ...
                <div className="modal-body p-4 pt-0 ">
                  <form className="mt-3 ">
                    <div className="form-floating mb-3">
                      <TextField error={emailError} required fullWidth label="Email" id="fullWidth" onChange={e => inputEmailHandler(e)} />
                      <small className='text-danger'>{emailErrorMessage}</small>
                    </div>
                    <div className="form-floating mb-3">
                      <TextField error={passwordError} required fullWidth label="Password" id="fullWidth-password-input" type="password"
